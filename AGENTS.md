...
| GET    | `/symbols/:symbolId/source`          | Read symbol source code          |
| GET    | `/files/:fileId`                      | Get one file                     |
| GET    | `/files/:fileId/symbols`             | Symbols in a file                |
| GET    | `/repos/:repoId/source`              | Read file source by path/fileId  |
| GET    | `/repos/find-by-name?q=`             | Find repo by name                |
| GET    | `/repos/find-by-path?q=`             | Find repo by path fragment       |
...
| `read_file_content`  | Read bounded source code from a file            |
| `read_symbol_source` | Read source code of a specific symbol           |
| `list_repos`         | List all registered repositories                |
| `find_repo`          | Discover repo by name or path fragment          |
...
### Step 2: Configure MCP in your agent
...
- command: `npm`
- args: `["run", "mcp"]`
- cwd: path to this `grap-local` repo
...
```bash
claude mcp add grap-local npm run mcp --cwd "C:/Users/LENOVO/OneDrive/Máy tính/grap-local"
```
